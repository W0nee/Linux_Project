...
int main(int argc, char *argv[])
{
	...
	sendto(client_socket, msg, sizeof(msg), 0, (struct sockaddr*)&server_addr, server_addr_size);
	
	// 원본 동영상 크기 수신
	int origin_file_size;
	recvfrom(client_socket, &origin_file_size, sizeof(int), 0, (struct sockaddr*)&server_addr, &server_addr_size);
	...
	while(recv_size != 0)
	{
		recv_size = recvfrom(client_socket, buf, BUF_SIZE, 0, (struct sockaddr*)&server_addr, &server_addr_size);
